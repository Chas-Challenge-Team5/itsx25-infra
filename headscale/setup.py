"""Render configuration or explicitly install the initial Headscale control plane.

The installer does not upgrade existing versions or overwrite changed configuration.
It never modifies SSH, OS Login, firewall rules, routing, or existing database files.
"""
...
def configuration(server_url, base_domain):
    ...
    return {
        ...
        "dns": {"magic_dns": True, "base_domain": base_domain,
                "override_local_dns": False, "nameservers": {"global": [], "split": {}}},
...
def install(source):
    ...
    expected = configuration(config["server_url"], config["dns"]["base_domain"])
    if config != expected:
...
def main():
    ...
    if args.command == "render":
        config = configuration(args.server_url, args.base_domain)
